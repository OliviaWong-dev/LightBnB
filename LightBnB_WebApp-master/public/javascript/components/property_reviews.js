$(() => {
  ...
  function addReviews(reviews) {
    clearReviews();
    $propertyReviews.append(
      `<h3>Reviews for ${reviews[0].property_title}</h3>`
    );
    const reviewHtml = reviews
      .map((review) => {
        const { id, review_rating, review_text, name, start_date, end_date } =
          review;
        ...
        <section class="property-review__details">
          ...
          <p>Stay from ${moment(start_date).format("ll")} to ${moment(
          end_date
        ).format("ll")}</p>
          <p>${review_text}</p> 
        </section>
    ...
    $propertyReviews.append(
      `<span class="property-reviews__return">Return to Listings</span>`
    );
